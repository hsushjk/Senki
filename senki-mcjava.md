...
2. 3dskinlayers（3D皮肤）
3. sodium（优化模组，优化fps）
4. FerriteCore（内存优化模组）
5. IMBlocker（输入法冲突修复）
6. Indium（渲染优化模组）
7. Krypton（又是一个一个一个优化模组，撅望）
8. LambDynamicLights（动态光源）
9. Mod Menu（模组菜单）
10. ReesesSodiumOptions（替换优化 Sodium 的选项屏幕）
11. Sodium Extra（Sodium拓展，将OptiFine的部分功能引入）
12. iris（光影加载）
...
23/06/12---把<万能皮肤修复补丁>干掉了，它不适合用户。

23/06/12---给客户端加了一堆优化模组，包括<动态光源>
